...
TEST_CASE("Parse XML with defined namespaces.", "[XML][Parse][Namespace]")
{
  ...
  SECTION("A root document and two namespaces defined in the child two table elements.", "[XML][Parse][Namespace]")
  {
    ...
    REQUIRE(XRef<XElement>(xml.root())[0].getNameSpace("h").value.getParsed() == "http://www.w3.org/TR/html4/");
    REQUIRE(XRef<XElement>(xml.root())[1].getNamespaceList().size() == 1);
    REQUIRE(XRef<XElement>(xml.root())[1].name() == "f:table");
    REQUIRE(XRef<XElement>(xml.root())[1].getNameSpace("f").value.getParsed() == "https://www.w3schools.com/furniture");
  }
  SECTION("A root document and two namespaces defined in the root element.", "[XML][Parse][Namespace]")
  {
    ...
    REQUIRE(XRef<XElement>(xml.root())[0].getNameSpace("h").value.getParsed() == "http://www.w3.org/TR/html4/");
    REQUIRE(XRef<XElement>(xml.root())[0].getNameSpace("f").value.getParsed() == "https://www.w3schools.com/furniture");
    REQUIRE(XRef<XElement>(xml.root())[1].name() == "f:table");
    REQUIRE(XRef<XElement>(xml.root())[1].getNamespaceList().size() == 2);
    REQUIRE(XRef<XElement>(xml.root())[1].getNameSpace("h").value.getParsed() == "http://www.w3.org/TR/html4/");
    REQUIRE(XRef<XElement>(xml.root())[1].getNameSpace("f").value.getParsed() == "https://www.w3schools.com/furniture");
  }
  SECTION(
  ...
  SECTION("A root document with a default namespace", "[XML][Parse][Namespace]")
  {
    ...
    REQUIRE(XRef<XElement>(xml.root())[0].getNamespaceList().size() == 1);
    REQUIRE(XRef<XElement>(xml.root())[0].getNameSpace(":").value.getParsed() == "http://www.w3.org/TR/html4/");
  }
