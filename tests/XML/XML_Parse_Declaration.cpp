...
TEST_CASE("Use XML object to parse XML declaration", "[XML][Parse][Declaration]")
{
  ...
  SECTION("Parse XML declaration with unsupported encoding. ", "[XML][Parse][Declaration]")
  {
    ...
    BufferSource source { xmlString };
    XML xml;
    REQUIRE_THROWS_WITH(xml.parse(source), "XML Syntax Error [Line: 1 Column: 39] Unsupported encoding UTF-32 specified.");
  }
  SECTION("Parse XML declaration with invalid standalone value. ", "[XML][Parse][Declaration]")
  {
    ...
    BufferSource source { xmlString };
    XML xml;
    REQUIRE_THROWS_WITH(xml.parse(source), "XML Syntax Error [Line: 1 Column: 56] Invalid standalone value of 'maybe'.");
  }
  SECTION("Parse XML declaration with extra content after root element. ", "[XML][Parse][Declaration]")
  {
    ...
    BufferSource source { xmlString };
    XML xml;
    REQUIRE_THROWS_WITH(xml.parse(source), "XML Syntax Error [Line: 2 Column: 18] Extra content at the end of document.");
  }
  ...
  SECTION("Parse empty XML declaration with content before root element.", "[XML][Parse][Declaration]")
  {
    ...
    BufferSource source { xmlString };
    XML xml;
    REQUIRE_THROWS_WITH(xml.parse(source), "XML Syntax Error [Line: 2 Column: 2] Content detected before root element.");
  }
  ...
  SECTION("Parse wrongly ordered attributes in XML declaration. ", "[XML][Parse][Declaration]")
  {
    ...
    BufferSource source { xmlString };
    XML xml;
    REQUIRE_THROWS_WITH(xml.parse(source), "XML Syntax Error [Line: 1 Column: 45] Incorrect order for version, encoding and standalone attributes.");
  }
