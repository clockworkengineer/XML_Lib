...
{
  throw XML_Lib::ValidationError(lineNumber, "Element <" + xElement.name() + "> " + error);
}
...
void DTD_Impl::checkElements(const XNode &xNode)
{
  if (xNode.isProlog()) {
    ...
    throw XML_Lib::ValidationError(lineNumber, "Invalid XMLNode encountered during validation.");
  }
}
