...
namespace XMLLib
{
    ...
    void DTD_Impl::parseValidNotations(const std::string &notations)
    {
        ...
        {
            if (m_xmlNodeDTD.parsed().getNotationCount(notation) == 0)
            {
    ...
    void DTD_Impl::parseValidateAttribute(const std::string &elementName, const DTDAttribute &dtdAttribute)
    {
        ...
        {
            if (m_xmlNodeDTD.parsed().getElement(elementName).idAttributePresent)
            {
                throw SyntaxError("Element <" + elementName + "> has more than one ID attribute.");
            }
            m_xmlNodeDTD.parsed().getElement(elementName).idAttributePresent = true;
        }
    ...
    void DTD_Impl::parseAttributeValue(ISource &dtdSource, DTDAttribute &attribute)
    {
        ...
        {
            dtdSource.ignoreWS();
            attribute.value = Core::parseValue(dtdSource, m_xmlNodeDTD.parsed().m_entityMapper);
    ...
    void DTD_Impl::parseAttributeList(ISource &dtdSource)
    {
        ...
        while (dtdSource.more() && Core::validNameStartChar(dtdSource.current()))
        {
            ...
            m_xmlNodeDTD.parsed().getElement(elementName).attributes.emplace_back(dtdAttribute);
            dtdSource.ignoreWS();
        }
    }
    /// <summary>
    ...
    void DTD_Impl::parseNotation(ISource &dtdSource)
    {
        ...
        m_xmlNodeDTD.parsed().addNotation(name, parseExternalReference(dtdSource));
        dtdSource.ignoreWS();
    }
    /// <summary>
    /// Parse DTD entity.
    ...
    void DTD_Impl::parseEntity(ISource &dtdSource)
    {
        ...
        if (dtdSource.current() == '\'' || dtdSource.current() == '"')
        {
            ...
            m_xmlNodeDTD.parsed().m_entityMapper.get(entityName).internal = entityValue.parsed;
        }
        else
        {
            m_xmlNodeDTD.parsed().m_entityMapper.get(entityName).external = parseExternalReference(dtdSource);
            if (dtdSource.match(U"NDATA"))
            {
                dtdSource.ignoreWS();
                m_xmlNodeDTD.parsed().m_entityMapper.get(entityName).notation = Core::parseName(dtdSource);
            }
        }
    }
    /// <summary>
    ...
    void DTD_Impl::parseElement(ISource &dtdSource)
    {
        ...
        {
            m_xmlNodeDTD.parsed().addElement(elementName, DTDElement(elementName, XMLValue{"EMPTY", "EMPTY"}));
        }
        else if (dtdSource.match(U"ANY"))
        {
            m_xmlNodeDTD.parsed().addElement(elementName, DTDElement(elementName, XMLValue{"ANY", "ANY"}));
        }
        else
        {
            std::string unparsed;
            ...
            m_xmlNodeDTD.parsed().addElement(elementName, DTDElement(elementName, parseElementContentSpecification(elementName, XMLValue{unparsed, ""})));
        }
        dtdSource.ignoreWS();
    }
    /// <summary>
    ...
    void DTD_Impl::parseParameterEntityReference(ISource &dtdSource)
    {
        ...
        BufferSource entitySource(m_xmlNodeDTD.parsed().m_entityMapper.translate(parameterEntity.unparsed));
    ...
    void DTD_Impl::parseDTD(ISource &dtdSource)
    {
        ...
        dtdSource.ignoreWS();
        m_xmlNodeDTD.parsed().setRootName(Core::parseName(dtdSource));
        // Parse in external DTD reference
        if (dtdSource.current() != '[')
        {
            m_xmlNodeDTD.parsed().setExternalReference(parseExternalReference(dtdSource));
        }
        ...
        if (dtdSource.current() == '[')
        {
            ...
            m_xmlNodeDTD.parsed().setType(DTD::DTDType::internal);
        }
        ...
        if (!m_xmlNodeDTD.parsed().getExternalReference().type.empty())
        {
            parseExternal(dtdSource);
            m_xmlNodeDTD.parsed().setType(m_xmlNodeDTD.parsed().getType() | DTD::DTDType::external);
        }
        ...
        for (auto &entityName : m_xmlNodeDTD.parsed().m_entityMapper.getList())
        {
            m_xmlNodeDTD.parsed().m_entityMapper.recursive(entityName.first, entityName.first[0]);
        }
        // Count lines in DTD
        m_xmlNodeDTD.parsed().setLineCount(static_cast<long>(std::count(m_xmlNodeDTD.unparsed().begin(), m_xmlNodeDTD.unparsed().end(), kLineFeed)) + 1);
    }
} // namespace XMLLib
