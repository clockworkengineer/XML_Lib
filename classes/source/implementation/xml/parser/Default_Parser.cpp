...
{
  return Node::make<Comment>(parseCommentBody(source));
}
...
{
  auto [name, parameters] = parsePIBody(source, false, Default_Parser::isNamespacesEnabled());
...
void Default_Parser::appendEntityOrContent(Node &xNode, const XMLValue &value, IEntityMapper &entityMapper, std::span<const XMLAttribute> inheritedNamespaces)
{
  if (value.isReference()) {
    ...
    if (content.isEntityReference()) {
      ...
      const auto *ns = getNodeNamespaces(xNode);
      const auto namespaces = ns ? *ns : inheritedNamespaces;
...
void Default_Parser::parseElementInternal(ISource &source, Node &xNode, IEntityMapper &entityMapper, std::span<const XMLAttribute> inheritedNamespaces)
{
  if (tryParseCommentOrPI(source, xNode)) {
    ...
    const auto *outerNs = getNodeNamespaces(xNode);
    const auto outerNamespaces = outerNs ? *outerNs : inheritedNamespaces;
...
Node Default_Parser::parse(ISource &source, const ParseOptions &options)
{
  ...
  if (match(source, "<")) {
    ...
    NamespaceValidator::validate(NRef<Element>(xmlRoot.getChildren().back()), source, isNamespacesEnabled(), isStrictNamespaces());
  } else {
